...
async function fetchLocationsInBox({ south, west, north, east, pageSize = PAGE_SIZE }) {
  const runBox = async (W, E) => {
    ...
    console.log('[fetchLocationsInBox] box', { W, E, count: out.length });
    return out;
  };
...
async function fetchViewportRows(map) {
  ...
  console.log('[fetchViewportRows] locs:', locs.length, 'rows:', rows.length);
  return rows;
}
...
function rowsToPointFeatures(rows) {
  ...
  for (const r of rows || []) {
    ...
    features.push({
      ...
      properties: { id: r.id, ts: r.timestamp, provider: r.provider, type: r.type },
    });
  }
...
export default function HexMap({ mode = 'hex' }) {
  ...
  useEffect(() => {
    ...
    map.on('load', async () => {
      ...
      map.addLayer({ id: 'hex-outline', type: 'line', source: 'hexes',
        paint: { 'line-color': '#0f766e', 'line-width': 1, 'line-opacity': 0.7 }});
      map.addLayer({ id: 'hex-fill-active', type: 'fill', source: 'hex-fills',
        paint: { 'fill-color': '#A7F3D0', 'fill-opacity': 0.35 }});
      map.addLayer({ id: 'hex-count-bubble', type: 'circle', source: 'hex-centers',
        paint: { 'circle-color': '#065f46', 'circle-radius': 14, 'circle-opacity': 0.95 }});
      map.addLayer({ id: 'hex-count-label', type: 'symbol', source: 'hex-centers',
      ...
      const openSheetForLngLat = (lngLat) => {
        const idx = h3.latLngToCell(lngLat.lat, lngLat.lng, HEX_RES);
        openHexSheet(idx);
      };
      ...
      map.on('mouseleave', 'clusters', unpointer);
      map.on('mouseenter', 'cluster-count', pointer);
      map.on('mouseleave', 'cluster-count', unpointer);
      map.on('mouseenter', 'unclustered-point', pointer);
      map.on('mouseleave', 'unclustered-point', unpointer);
      map.on('mouseenter', 'hex-fill-active', pointer);
      map.on('mouseleave', 'hex-fill-active', unpointer);
      map.on('mouseenter', 'hex-count-bubble', pointer);
      map.on('mouseleave', 'hex-count-bubble', unpointer);
      map.on('mouseenter', 'hex-count-label', pointer);
      map.on('mouseleave', 'hex-count-label', unpointer);
  ...
  useEffect(() => {
    (async () => {
      await supabase.from('measurements').select('id', { count: 'exact', head: true });
      await supabase.from('locations').select('id,measurement_id,lat,lon').limit(1);
    })();
  }, []);
